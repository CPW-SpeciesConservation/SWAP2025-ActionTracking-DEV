  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$action_table <- renderDT({
      datatable(action_data(), selection = "single", rownames = FALSE, 
                options = list(pageLength = 5, dom = 'ftp', columnDefs = list(list(visible = FALSE, targets = c(0, 1)))))
    })
    ...
    # Auto-update status dropdown to match selected action
